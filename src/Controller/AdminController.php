...
class AdminController extends AbstractController
{
    ...
    public function modif_article(Article $article, Request $request, EntityManagerInterface $manager)
    {
        ...
        $form = $this->createForm(ArticleType::class, $article, array('modifer' => true));
     ...
     */
    public function ajout_categorie(Request $request, EntityManagerInterface $manager)
    {
     ...
     */
    public function liste_categorie(CategorieRepository $repository)
    {

        $categories = $repository->findAll();
        return $this->render("admin/liste_categorie.html.twig", [
     ...
     */
    public function commande(PanierService $panierService, EntityManagerInterface $manager, SessionInterface $session,  $param=null)
    {
        $panier = $panierService->getFullPanier();
    ...
    public function send_mail(Request $request)
    {
        ...
        $mess = $request->request->get('messqge');
        ...
        $mailer->send($message);
        $this->addFlash('success', 'Votre Email a bien été envoyé. Il sera traité dans les meilleurs délais.');
     ...
     */
    public function liste_promos(PromoRepository $promoRepository)
    {
    ...
    /**
     * @Route ("/modif_pormo/{id}", name="modif_promo")
    ...
    public function ajout_promo(Request $request, EntityManagerInterface $manager, userRepository $userRepository)
    {
        ...
        $form = $this->createForm(PromoType::class, $promo, array('ajouter' => true));
        $form->handleRequest($request);
        ...
        if ($form->isSubmitted() && $form->isValid()):
            ...
            foreach ($users as $user):
                ...
                $mess = "Bonne nouvelle !! Du $dated au $datef pour tout achat d'un montant minimum de $montant €, vous bénéficier d'une remise de $remise € avec le code promotionnel suivant : $promotion. Venez vite en profiter ! ";
            ...
            $manager->flush();
            $this->addFlash("success", "La promotion a bien été créée");
    ...
    public function verif_promo(Request $request, SessionInterface $session, PromoRepository $promoRepository, PanierService $panierService)
    {
        ...
        if($promo):
            ...
            $totalRemise=$panierService->getTotalRemise($promo[0]->getRemise());
            $this->addFlash('success','Votre code promotionnel a bien été pris en compte.');
          ...
          return $this->render('front/panier.html.twig',[
              ...
              'total'=>$total
                ...
                $this->addFlash('alert','Votre code promotionnel n\'est pas valide.');
                return $this->redirectToRoute('cart');
            endif;
        endif;
        return $this->redirectToRoute('cart');
